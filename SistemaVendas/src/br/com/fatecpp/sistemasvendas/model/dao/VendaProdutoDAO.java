...
import br.com.fatecpp.sistemasvendas.model.domain.VendaProduto;
...
public class VendaProdutoDAO implements DAOGenerico<VendaProduto> {
    
    private static VendaProdutoDAO unicaInstancia;
    private EntityManager em;//é um objeto que ira gerenciar as entidades
            
    public static VendaProdutoDAO getUnicaInstancia(){
        if(unicaInstancia == null){
            unicaInstancia = new VendaProdutoDAO();
        }
        return unicaInstancia;
    }
    
    
    private VendaProdutoDAO(){
    ...
    @Override
    public void inserir(VendaProduto entidade) {
        try{
    ...
    public void excluir(VendaProduto entidade) {
        try{
            em.getTransaction().begin();
            VendaProduto vp = em.merge(entidade);
            em.remove(entidade);
            em.getTransaction().commit();
        }catch(Exception ex){
            System.out.println(ex.getMessage());
    ...
    @Override
    public List<VendaProduto> consultar(String opcao, String parametro) {
        Query q = null;
